...
export default function BlogEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ Post aktualisiert')
      navigate('/admin/blog')
    } catch (error: any) {
      console.error('Error:', error)
      setError(error.message || 'Fehler beim Aktualisieren des Posts')
    } finally {
      setSaving(false)
    }
  }
        ...
        <Box sx={{ maxWidth: 900, mx: 'auto' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              >
                {saving ? <CircularProgress size={24} sx={{ color: '#fff' }} /> : 'Änderungen speichern'}
              </Button>
              <Button
